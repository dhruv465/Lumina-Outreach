...
const RealTimeSTT: React.FC<RealTimeSTTProps> = ({ 
  ...
  const [connectionStatus, setConnectionStatus] = useState<'idle' | 'connected' | 'error' | 'verifying' | 'verified'>('idle');
  ...
  useEffect(() => {
    ...
    const loadApiKey = async () => {
      try {
        ...
        const foundApiKey = config.deepgramApiKey || 
        ...
        if (foundApiKey) {
          ...
          toast({
            ...
            description: "Deepgram API key is not configured. Please add it in the Configuration page.",
            variant: "destructive"
          });
        }
      } catch (error) {
    ...
    try {
      ...
      if (!apiKey) {
        toast({
          ...
          description: "Deepgram API key is required. Please configure it in the Configuration page.",
          variant: "destructive"
        });
        setIsLoading(false);
      ...
      try {
        ...
        socket.onerror = (error) => {
          ...
          toast({
            ...
            description: "Failed to connect to Deepgram service. Check your API key and network connection.",
            variant: "destructive"
          });
          setIsLoading(false);
        ...
        toast({
          ...
          description: "Please allow microphone access to use speech-to-text functionality.",
          variant: "destructive"
        });
        setIsLoading(false);
        setIsRecording(false);
        ...
        {connectionStatus === 'verifying' && (
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-4 flex items-center">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Connecting to Deepgram API...</p>
            </div>
          </div>
        )}
        ...
        {connectionStatus === 'verified' && (
          <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4 mb-4 flex items-center">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Successfully connected to Deepgram API</p>
            </div>
          </div>
        )}
        ...
        {connectionStatus === 'error' && (
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4 mb-4 flex items-center">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Failed to connect to Deepgram API. Please check your API key.</p>
            </div>
          </div>
        )}
          ...
          <div className="h-2 w-2 rounded-full mr-2"
            style={{ 
              backgroundColor: 
                ...
                connectionStatus === 'verified' ? '#22c55e' :
                connectionStatus === 'verifying' ? '#3b82f6' :
                connectionStatus === 'error' ? '#ef4444' : 
                '#a1a1aa'
            }}
          />
          <span className="text-sm text-muted-foreground">
            {connectionStatus === 'connected' ? 'Connected to Deepgram' :
             connectionStatus === 'verified' ? 'Verified connection to Deepgram' :
